        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header card-header-primary card-header-icon d-flex justify-content-between">
                    ...
                    <a href="{{ route('admin-salas.create') }}" class="btn btn-primary btn-sm mt-2">
                        <i class="material-icons">add</i> Nueva Sala
                    </a>
                        ...
                        <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0"
                            ...
                            <tbody>
                                @foreach ($data['salas'] as $item)
                                    <tr>
                                        <td>{{ $item->id_sala }}</td>
                                        <td class='text-center'><img src="{{ $item->image }}" alt="{{ $item->nombre }}"
                                                width='50' height="50" /></td>
                                        <td>{{ $item->nombre }}</td>
                                        <td>{{ $item->cupo }}</td>
                                        <td>
                                            <a href="{{ route('admin-salas.edit', $item) }}"
                                                class="btn btn-link btn-info btn-just-icon editar" data-toggle="tooltip"
                                                title="Editar"><i class="material-icons">edit</i></a>
                                            <form id="eliminarForm-{{ $item->id_sala }}"
                                                action="{{ route('admin-salas.destroy', $item) }}" method="post"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                                    onclick="confirmarEliminacion('{{ $item->id_sala }}')"
